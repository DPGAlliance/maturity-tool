...
from ui import dispay_repo_info, display_brach_results
from data import get_branches_cached
from maturity_tools.analyzers import BranchAnalyzer
...
def main():
    ...
    st.write("This app is for showcasing the curretly available data from the maturity_tools package.")
    ...
    dispay_repo_info(info_result)
    st.divider()
    
    st.subheader("Branches")
    branches_df = get_branches_cached(owner, repo, GITHUB_TOKEN)
    display_brach_results(branches_df)
    ...
    st.write(f"There are {stale} stale branches, and {active} active ones. Looking at the last {days} days.")
    st.divider()

    # This is all general stuff so far. Lets get into branch specific analysis next.
    selected_branch = st.selectbox("Select a branch to analyze further", branches_df['branch_name'].tolist())
    st.subheader(f"Commits on {selected_branch} branch")
